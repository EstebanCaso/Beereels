...
public class GameManager : MonoBehaviour
{
    ...
    public float penaltyPerRoadSecond = 3f;         // Penalizaci�n por ver la carretera

    // Estado de puntos
    private float _totalScore = 0f;
    private float _survivalTime = 0f;

    // Estado de "qu� est� haciendo el jugador ahora"
    ...
    // Singleton simple para que otros scripts lo encuentren f�cil
    ...
    void Update()
    {
        ...
        // Penalizaci�n por ver la carretera
        if (_isLookingAtRoad)
    ...
    // Llamado desde el sistema de detecci�n de mirada
    ...
    public void LoadMainMenu()
    {
        ...
        {
            Debug.LogWarning("[GameManager] MainMenu scene not in build settings. Reloading current scene as fallback.");
            UnityEngine.SceneManagement.SceneManager.LoadScene(
                UnityEngine.SceneManagement.SceneManager.GetActiveScene().name
            );
        }
